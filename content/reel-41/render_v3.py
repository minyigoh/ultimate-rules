...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Indirect fouls", "Someone crashes into you in the stack. The disc is on the far side of the field. Nobody near you was going for it. Is that a foul?", 41, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('contact', g_main(2, 'CONTACT OFF THE DISC', "A real foul, in a place the disc never reached.", "Two players collide in the stack. The disc is on the other side of the field and nobody near the contact was going for it. That is still a foul, and it has its own name. It needs the contact to be non-minor, and it needs the contact to be away from the play — if it affected somebody's attempt on the disc, it is an ordinary receiving foul instead.", ['17.8.1'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('contact_r', g_detail(3, [('17.8.1', [rt('17.8.1')])]), [0.3, 2.0]),
    ('wait', g_main(4, 'THE TWO-SECOND WAIT', "You are allowed to find out whether it mattered.", "You do not have to call it the moment it happens. You get up to two seconds to work out whether the contact is going to affect the play at all. Word for word the same allowance a pick gets, for the same reason: most contact away from the disc turns out to have cost nobody anything.", ['17.8.1.1'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('wait_r', g_detail(5, [('17.8.1.1', [rt('17.8.1.1')])]), [0.3, 2.0]),
    ('back', g_main(6, 'MAKE UP THE GROUND', "You get the position back. That is the whole remedy.", "If the call stands, the fouled player makes up whatever position the contact cost them, and play restarts with a check. That is the entire remedy. No disc, no yardage, no change to the stall count. You are put back where you were going, which is also why there is nothing here worth angling for.", ['17.8.2'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('back_r', g_detail(7, [('17.8.2', [rt('17.8.2')])]), [0.3, 2.0]),
    ('tip', g_tip(8, "Most off-disc contact costs you nothing.", "Most off-disc contact resolves itself and needs no call at all. Take the two seconds. If you got up, carried on, and arrived where you were going anyway, let it go. Call it when the contact actually stopped you getting somewhere."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 41), [0.3, 0.8, 1.0, 1.4]),
]
