...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Offside and false start on the pull", "Two calls with usefully different remedies, and both have a deadline.", 55, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('breach', g_main(2, 'TWO WAYS TO BREAK THE SET', "One team moves. The other crosses.", "A false start is the offence moving after signalling readiness, either off the goal line or relative to each other. Offside is the defence crossing that line before the pull.", ['7.3', '7.4'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('breach_r', g_detail(3, [('7.3', [rt('7.3')]), ('7.4', [rt('7.4')])]), [0.3, 1.7, 2.0]),
    ('window', g_main(4, 'CALL IT BEFORE THE TOUCH', "Call it before the disc is touched.", "Either breach has to be called before the offence touches the disc. Once a receiver has a hand on it the moment has gone, and the point plays on exactly as it stands.", ['7.5'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('window_r', g_detail(5, [('7.5', [rt('7.5')])]), [0.3, 2.0]),
    ('remedy', g_main(6, 'TWO CALLS, TWO RESTARTS', "The restart depends on who called it.", "Defence calls false start: the thrower sets a pivot and play restarts there as if a time-out had been called. Offence calls offside: let it land, then take it as a brick.", ['7.5.1', '7.5.2'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('remedy_r', g_detail(7, [('7.5.1', [rt('7.5.1')]), ('7.5.2', [rt('7.5.2')])]), [0.3, 1.7, 2.0]),
    ('tip', g_tip(8, "Call offside, then let it land.", "If you call offside, shout it and then deliberately do not touch the disc. Catching the pull forfeits the call: the restart in 7.5.2 only exists if the disc hits the ground untouched, and a teammate reaching out of habit is how it gets lost."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 55), [0.3, 0.8, 1.0, 1.4]),
]
