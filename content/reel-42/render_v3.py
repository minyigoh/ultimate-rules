...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Where you pick the disc up after a turnover", "Turnover. The disc is lying somewhere on the field. Where do you actually put your pivot?", 42, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('stopped', g_main(2, 'WHERE IT STOPPED', "In the middle of the field, the spot is the spot.", "The turnover location is where the disc came to rest, or where an offensive player picked it up. If that is in the central zone — the middle of the field, between the two end zones — then that is your pivot, exactly there. No walking it anywhere. Most turnovers are this case and need no thought at all.", ['13.9'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('stopped_r', g_detail(3, [('13.9', [rt('13.9')])]), [0.3, 2.0]),
    ('notstopped', g_main(4, 'NOT WHERE IT STOPPED', "Two cases where you walk to a line instead.", "Two cases move you. If the disc is in the end zone you are attacking, you walk to the nearest point on the goal line. If it went out-of-bounds, you go to the nearest point on the central zone to where it crossed the line. What they have in common is that the spot is not where the disc ended up.", ['13.10', '13.8'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('notstopped_r', g_detail(5, [('13.10', [rt('13.10')]), ('13.8', [rt('13.8')])]), [0.3, 1.7, 2.0]),
    ('agree', g_main(6, 'IF YOU CANNOT AGREE', "There is a rule for that, and it takes three seconds.", "Two people pace out two different spots and neither will move. This happens more than you would think, and the rulebook does not ask you to argue it out. Take the midpoint between the two proposals and restart. It is a rule, so nobody has to concede anything.", ['8.5.4'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('agree_r', g_detail(7, [('8.5.4', [rt('8.5.4')])]), [0.3, 2.0]),
    ('tip', g_tip(8, "The spot is where it crossed the line, not where it rolled to a stop.", "The common mistake is walking straight in from where the disc finished. A throw that sails out and rolls another fifteen metres comes back to where it crossed the line, not to where you picked it up. Watch the crossing point, not the disc."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 42), [0.3, 0.8, 1.0, 1.4]),
]
