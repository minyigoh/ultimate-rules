...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "When a call is made, everybody freezes", "Somebody calls foul, and half the field keeps running. That's not one mistake. That's a second rule broken on top of the first.", 35, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('stop', g_main(2, 'PLAY STOPS INSTANTLY', "Nothing can turn over after a call.", "The instant a call is made, play stops and no turnover is possible. Not a drop that happens while people are still talking, not a disc that hits the ground because one defender kept covering. The rule names three exceptions by number, and the main one — a disc already in the air — is tomorrow's lesson.", ['16.1'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('stop_r', g_detail(3, [('16.1', [rt('16.1')])]), [0.3, 2.0]),
    ('echo', g_main(4, 'ECHO THE CALL', "A call nobody heard didn't stop play.", "The rule puts the work on everybody, not just the caller. You stop play by visibly or audibly communicating the stoppage, and every player on the field should echo the call. On a full pitch the far end cannot hear one voice. And if play stops for a discussion with no call at all, a call is deemed made when the discussion started.", ['15.7'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('echo_r', g_detail(5, [('15.7', [rt('15.7')])]), [0.3, 2.0]),
    ('freeze', g_main(6, 'GO BACK AND FREEZE', "Back to where you were. Then stop.", "Where you were when the call was made — not where you were heading. Then you stand still until the disc is checked in. Every player, both teams, not only the two arguing. Drifting a few metres into a better lane while a call is sorted out is the most common beginner error there is, and it is a breach of the check in its own right.", ['10.2.1', '10.2.3'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('freeze_r', g_detail(7, [('10.2.1', [rt('10.2.1')]), ('10.2.3', [rt('10.2.3')])]), [0.3, 1.7, 2.0]),
    ('tip', g_tip(8, "Freeze first. Argue after.", "Stop moving the instant you hear a call, even if you are certain it is wrong. You can contest it a second later, from exactly where you are standing. Drifting into better position while the call is being sorted out turns one disagreement into two."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 35), [0.3, 0.8, 1.0, 1.4]),
]
