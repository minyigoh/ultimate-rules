...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Continuation: when the disc is already in the air", "You called it, and the disc is already up. This is the one time the rulebook wants you to keep playing.", 36, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('trigger', g_main(2, "PLAY DOESN'T STOP", "A call in the air doesn't freeze the play.", "Yesterday's rule stops play the instant somebody calls. This is its main exception. If the call is made against the thrower who then throws, or by the thrower mid-throw, or while the disc is already in the air, nothing is decided yet. The outcome is settled once possession has been established.", ['16.2', '16.2.3'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('trigger_r', g_detail(3, [('16.2', [rt('16.2'), ('16.2.3', rt('16.2.3'))])]), [0.3, 2.0]),
    ('stands', g_main(4, 'YOUR CATCH, YOUR PLAY', "Catch it anyway and the play stands.", "If the team that called it gains or retains possession as a result of the pass, the play stands. You do not get to rewind to a call you no longer need. The caller can even say \u201cPlay on\u201d as soon as possible, and then nothing stops at all.", ['16.2.4.1'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('stands_r', g_detail(5, [('16.2.4.1', [rt('16.2.4.1')])]), [0.3, 2.0]),
    ('stops', g_main(6, 'OTHERWISE, STOP', "Lose it, and play stops.", "If the calling team does not gain or retain possession as a result of the pass, play must be stopped. Then one question decides where the disc goes. If the calling team believes the breach affected possession, the disc is returned to the thrower for a check.", ['16.2.4.2', '16.2.4.2.1'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('stops_r', g_detail(7, [('16.2.4.2', [rt('16.2.4.2')]), ('16.2.4.2.1', [rt('16.2.4.2.1')])]), [0.3, 1.7, 2.0]),
    ('tip', g_tip(8, "Call it, then finish the play.", "Stopping to argue mid-flight is how teams lose discs. Make the call, keep running, catch it. The rule is written so a call cannot cancel an outcome you did not like, and so it cannot cost you one you earned."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 36), [0.3, 0.8, 1.0, 1.4]),
]
