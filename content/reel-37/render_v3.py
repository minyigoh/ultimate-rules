...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "\"It didn't affect the play\"", "A rule that outranks every other rule in the book — including the one you just used to make your call.", 37, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('agree', g_main(2, 'BOTH TEAMS AGREE', "Changed nothing? Then it stands.", "If the players involved from both teams agree that the event or the call did not affect the outcome, the play stands. The rulebook then says something it says almost nowhere else: this rule is not superseded by any other rule. It does not matter when the call came, or whether the call was correct.", ['16.3'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('agree_r', g_detail(3, [('16.3', [rt('16.3')])]), [0.3, 2.0]),
    ('goal', g_main(4, 'GOAL, OR RESET', "The goal stays. Otherwise, check it in.", "If the play resulted in a goal, the goal stands. If it did not, the affected players may make up any positional disadvantage the event or call caused, and play restarts with a check. Nobody rewinds thirty seconds of good play to settle something that changed nothing.", ['16.3.1', '16.3.2'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('goal_r', g_detail(5, [('16.3.1', [rt('16.3.1')]), ('16.3.2', [rt('16.3.2')])]), [0.3, 1.7, 2.0]),
    ('tip', g_tip(6, "Ask the question out loud.", "\"Did that affect it?\" — \"No, play on.\" That exchange should be the most common conversation you have on the field. It settles more disputes than being right does, and when the answer really is no, agreeing costs you nothing."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(7, 37), [0.3, 0.8, 1.0, 1.4]),
]
