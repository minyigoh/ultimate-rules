...
SCENES = [
    ('cover', g_cover(1, 'INTERMEDIATE', "When several things go wrong at once", "The tiebreaker for messy, multi-call plays.", 58, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('sequence', g_main(2, 'LATEST FIRST, EARLIEST LAST', "Multiple breaches unwind backwards.", "When more than one breach happens on the same play, you do not argue about which mattered most. You unwind them in reverse: latest breach first, earliest breach last.", ['15.12'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('sequence_r', g_detail(3, [('15.12', [rt('15.12')])]), [0.3, 2.0]),
    ('offset', g_main(4, 'BOTH TEAMS FOULED', "Fouls both ways cancel out.", "If the offence and the defence both have accepted fouls on the same play, neither one wins. They offset, and the disc goes back to the last thrower nobody disputed.", ['17.9.1'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('offset_r', g_detail(5, [('17.9.1', [rt('17.9.1')])]), [0.3, 2.0]),
    ('samepoint', g_main(6, 'SAME POINT, SAME MOMENT', "Nobody initiated it, so nobody loses it.", "Sometimes two opposing players move to the same spot at the same instant and neither of them started the contact. Non-minor contact from that is treated as offsetting fouls.", ['17.9.2'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('samepoint_r', g_detail(7, [('17.9.2', [rt('17.9.2')])]), [0.3, 2.0]),
    ('tip', g_tip(8, "Go back to the last undisputed thrower.", "In a genuinely tangled play, returning the disc to the last undisputed thrower is almost always the fair answer. Reach for it before the discussion becomes a reconstruction of who touched whom."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 58), [0.3, 0.8, 1.0, 1.4]),
]
