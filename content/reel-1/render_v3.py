...
RULES_JSON = "/sessions/dazzling-laughing-clarke/mnt/Learn Ultimate Frisbee/content/rules.json"
...
def g_cover(no):
    ...
    y = 600; head = []
    for line in ["The whole game", "in one paragraph"]:
        head.append(bold(MARGIN, y, line, 96, CREAM, sw=3.2)); y += 108
    groups.append(head)
    y += 40; sub = []
    for line in wrap_lines("Before anything else makes sense, you need the shape of the whole game.", 36):
        sub.append(reg(MARGIN, y, line, 36, CREAM, opacity=0.75)); y += 48
    groups.append(sub)
    groups.append([bold(MARGIN, H-140, "LESSON 1 / 75", 34, ORANGE, sw=1.6)])
    return groups
...
def g_tip(no):
    ...
    y = 610; head = []
    for line in wrap_lines("Look up first", 66, ratio=0.56):
        head.append(bold(MARGIN, y, line, 66, CREAM, sw=3.0)); y += 78
    groups.append(head)
    y += 46; body = []
    for line in wrap_lines("New players freeze after catching. Don't — you've got ten seconds, longer than it feels.", 36):
        body.append(reg(MARGIN, y, line, 36, CREAM, opacity=0.8)); y += 50
    groups.append(body)
    return groups
...
SCENES = [
    ...
    ("obj",     g_main(2, "THE OBJECTIVE", "Two teams. One end zone.",
                 "Two teams of seven. Score by catching the disc inside the end zone you're attacking — that's the entire objective.",
                 ["4.1", "14.1"], 1),                                 [0.30, 0.45, 0.70, 1.50, 0.80]),
    ("obj_r",   g_detail(3, [("4.1",  [rt("4.1")]),
                             ("14.1", [rt("14.1"), ("14.1.1", rt("14.1.1"))])]),
                                                                       [0.30, 1.70, 2.60]),
    ("throw",   g_main(4, "THE THROW", "Catch, plant, throw.",
                 "You can't run with the disc. One foot plants as your pivot; the other goes anywhere. Any direction is legal, even backwards.",
                 ["18.2.2"], 2),                                       [0.30, 0.45, 0.70, 1.50, 0.80]),
    ("throw_r", g_detail(5, [("18.2.2", [rt("18.2.2")])]),             [0.30, 2.00]),
    ("flip",    g_main(6, "THE FLIP", "Drop it and it's theirs.",
                 "Incomplete pass, interception, or out-of-bounds — possession flips instantly, right where it happened. No whistle, no reset.",
                 ["13.1"], 3),                                         [0.30, 0.45, 0.70, 1.50, 0.80]),
    ("flip_r",  g_detail(7, [("13.1", [rt("13.1"),
                                       ("13.1.1", rt("13.1.1")),
                                       ("13.1.2", rt("13.1.2"))])]),  [0.30, 2.90]),
    ("tip",     g_tip(8),                                              [0.30, 0.45, 0.70, 1.70]),
    ("close",   g_closing(9),                                          [0.30, 0.80, 1.00, 1.40]),
]
