...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Calling a time-out you don't have", "Your team has none left. You call one anyway. Here is what it costs.", 49, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('stops', g_main(2, 'PLAY STOPS ANYWAY', "The call still works. That is the trap.", "You form the T, you call it, and play stops — even though your team has none left. The rulebook does not ignore the call. It stops the point, and then it charges you for it.", ['20.4'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('stops_r', g_detail(3, [('20.4', [rt('20.4')])]), [0.3, 2.0]),
    ('two_sec', g_main(4, 'TWO SECONDS ON THE COUNT', "Two are added to the restart count.", "Not two onto the count at the moment you called it. Two onto the number the marker would otherwise have restarted play on. Play resumes with a check.", ['20.4'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('ten', g_main(5, 'TEN IS A TURNOVER', "Late in a count, it costs the disc.", "If those two seconds take the restart to ten or above, that is a stall-out turnover. A restart at eight becomes ten. The later the count, the less this is a small penalty.", ['20.4'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('tip', g_tip(6, "Know the number before you need it.", "Time-outs get spent in the first half and forgotten by the second. Your captain should say how many are left after every one, and the thrower should know it without asking. The moment you want a time-out is the worst moment to find out you have none."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(7, 49), [0.3, 0.8, 1.0, 1.4]),
]
