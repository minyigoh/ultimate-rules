...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "What the stall count restarts at", "Play stopped at stall six. It does not restart at six. It probably does not restart at one either, and most players guess.", 39, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('whose', g_main(2, 'WHOSE BREACH WAS IT', "Their fault, back to one. Your fault, maximum nine.", "The count does not pick up where it left off. It is re-set, and which number you re-set to depends entirely on who caused the stoppage. If the defence committed the breach, the thrower gets the whole count back. If the offence committed it, they get almost none of it back.", ['9.5.1', '9.5.2'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('whose_r', g_detail(3, [('9.5.1', [rt('9.5.1')]), ('9.5.2', [rt('9.5.2')])]), [0.3, 1.7, 2.0]),
    ('other', g_main(4, 'EVERY OTHER CALL', "Picks and most calls: maximum six.", "Most calls are neither. A pick, a contested foul, an injury: all of them restart at maximum six. The rule adds a 'However' pointing at two narrow exceptions — a receiving breach alongside a call on the thrower, and a violation during the check — and neither is covered here. A contested stall-out is its own number, because that call is about the count itself.", ['9.5.5', '9.5.3'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('other_r', g_detail(5, [('9.5.5', [rt('9.5.5')]), ('9.5.3', [rt('9.5.3')])]), [0.3, 1.7, 2.0]),
    ('maximum', g_main(6, "WHAT \"MAXIMUM\" MEANS", "The last number said, plus one — or n. Whichever is lower.", "Maximum six is not six. It means the last number fully uttered plus one, or six, whichever of the two is lower. On four when the pick came? Four plus one is five, and five is lower, so you restart at five. On eight? Nine against six, six is lower, so you go back to six.", ['9.6.1'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('maximum_r', g_detail(7, [('9.6', [rt('9.6'), ('9.6.1', rt('9.6.1'))])]), [0.3, 2.0]),
    ('tip', g_tip(8, "Say the number out loud before anyone argues.", "Maximum never jumps you forward. It can only hold you where you were or pull you back, which is why the marker gains nothing by arguing for it. Agree the last number out loud before you argue about the call, and the restart takes three seconds instead of thirty."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 39), [0.3, 0.8, 1.0, 1.4]),
]
