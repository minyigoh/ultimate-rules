...
def cover_slide():
    ...
    y = 600
    # Line break, not the title: "Week seven: the time" measured 974.6 of the
    # 900px column at 96px. Broken here it is 750.6 / 880.3. Fixed at the draft
    # gate on 2026-09-17; the title itself is unchanged.
    for line in ["Week seven: the", "time between plays"]:
        body.append(bold(MARGIN, y, line, 96, CREAM, sw=3.2))
        y += 108
    y += 40
    sub = "This week's seven lessons — everything the daily reels covered, 17–23 September."
...
def closing_slide():
    ...
    y = 520
    # Forty-nine, the last lesson IN THE BLOCK. Lesson 49 posts 2026-09-23, the
    # day before this deck (2026-09-24), so the count is exact on the day.
    for line in ["That's forty-nine", "of seventy-five."]:
...
# ---- the week being recapped: lessons 43-49 ----
# A clean contiguous seven, opening where carousel-post-7 stopped. Lesson 28
...
recaps = [
    (43, "Turnover in your own end zone: you choose",
     "Taking it to the line gives your team more field to work with. Staying deep can catch an unset defence. Decide before you pick it up.",
     ["13.11", "13.11.1", "13.11.2", "13.11.2.1", "13.11.3"]),
    (44, "You have to fetch the disc promptly",
     "Defence isn't allowed to obstruct you from getting to the disc or setting your pivot. If they're in the way, say so.",
     ["8.5", "8.5.1.1", "8.5.1.2", "8.5.2", "8.5.2.1", "8.5.3"]),
    (45, "Live play and dead play",
     "Dead play is your thinking time. Use the walk to the pivot to look downfield and pick your first throw.",
     ["8.1", "8.1.1", "8.1.2", "8.1.3", "8.1.4", "8.2", "8.3"]),
    (46, "Don't touch a pull you can't catch",
     "The rule of thumb: if you're not certain you'll catch it cleanly, let it land. You lose nothing.",
     ["7.8", "13.1.4", "7.10"]),
    (47, "Pulls that go out of bounds: the brick",
     'Signal the brick by raising one arm and calling "brick" as you walk to the mark. Say it before you pick the disc up.',
     ["7.12", "2.5", "7.11"]),
    (48, "Calling a time-out",
     "How many time-outs you get isn't in the core rules — it's set by your event's format. Ask your captain before the first game.",
     ["20.1", "20.2", "20.3", "20.3.1", "20.3.4", "20.3.5", "20.3.6"]),
    (49, "Calling a time-out you don't have",
     "Know your team's remaining time-outs. It's the captain's job to keep everyone updated during the game.",
     ["20.4"]),
]

SLUGS = {43: "own_end_zone", 44: "fetch_promptly", 45: "live_and_dead",
         46: "dropped_pull", 47: "the_brick", 48: "time_out",
         49: "no_time_outs_left"}
