...
SCENES = [
    ('cover', g_cover(1, 'NEVER PLAYED BEFORE', 'There are no referees. You are the referee.', 'This is the one thing that makes ultimate different from every other field sport.', 5), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('self', g_main(2, 'SELF-OFFICIATED', 'No referees. You make the call.', "Every call on the field — including against your own team — is made by the players themselves. Ultimate runs on the assumption that you'll be honest.", ['1.1'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('self_r', g_detail(3, [('1.1', [rt('1.1')])]), [0.3, 2.0]),
    ('gentle', g_main(4, 'GENTLE BY DESIGN', "Recreate, don't punish.", "The rules assume nobody breaks them on purpose. That's why penalties are gentle — they try to recreate what would've happened, not punish you for it.", ['1.2'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('gentle_r', g_detail(5, [('1.2', [rt('1.2')])]), [0.3, 2.0]),
    ('who', g_main(6, 'WHO CAN CALL IT', 'Only the player fouled.', "Only the player who was fouled can call the foul. You can't call it on a teammate's behalf, even if you saw it clearly.", ['15.4'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('who_r', g_detail(7, [('15.4', [rt('15.4')])]), [0.3, 2.0]),
    ('disagree', g_main(8, 'IF YOU DISAGREE', 'Discuss, or it goes back.', "If a call is contested, play stops. Can't agree after discussion? The disc goes back to the last thrower both sides agree on.", ['13.3'], 4), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('disagree_r', g_detail(9, [('13.3', [rt('13.3')])]), [0.3, 2.6]),
    ('tip', g_tip(10, 'Call it yourself', "Only the player who was fouled can call the foul. You can't call it on a teammate's behalf."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(11, 5), [0.3, 0.8, 1.0, 1.4]),
]
