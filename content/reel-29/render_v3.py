...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Strip fouls", "You caught it. Contact knocked it out of your hands. That is a specific foul with a specific name, and it is not the same as being fouled on the catch.", 29, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('caught', g_main(2, 'YOU HAD IT, AND IT CAME OUT', "A strip is a foul that costs you a catch you'd already made.", "The word doing the work is caught. Lesson 28's receiving foul is contact that stops you completing a catch. A strip is contact that takes away a catch you had already completed \u2014 the disc was yours, and the foul is why it isn't any more.", ['17.3.1'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('caught_r', g_detail(3, [('17.3.1', [rt('17.3.1')])]), [0.3, 2.0]),
    ('goal', g_main(4, 'IF IT WOULD HAVE BEEN A GOAL', "You get the goal, not the disc.", "If the reception would otherwise have been a goal and the foul is accepted, a goal is awarded \u2014 not possession on the goal line. Anywhere else on the field it resolves like any receiving foul: you keep it where it happened, and restart with a check.", ['17.3.2'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('goal_r', g_detail(5, [('17.3.2', [rt('17.3.2')])]), [0.3, 2.0]),
    ('tip', g_tip(6, "Say \"strip\", not just \"foul\".", "The consequence is different, so the name should be too. \"Foul\" points at lesson 28's remedy; \"strip\" points at this one \u2014 and on a catch in the end zone, that is the difference between a check and a point."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(7, 29), [0.3, 0.8, 1.0, 1.4]),
]
