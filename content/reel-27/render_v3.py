...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Not every touch is a foul", "You brushed arms going for the same disc. That's not a foul, and the rulebook says so in one sentence.", 27, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('tolerance', g_main(2, 'THE TOLERANCE IS DELIBERATE', "Two people going to one point will sometimes touch.", "The tolerance is written into the rules, not extended to you as a courtesy. Minor contact should be minimized \u2014 and, separately, it is not considered a foul. Two people running hard at the same spot is the sport working correctly, not breaking.", ['12.8'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('tolerance_r', g_detail(3, [('12.8', [rt('12.8')])]), [0.3, 2.0]),
    ('nonminor', g_main(4, 'A FOUL NEEDS MORE THAN CONTACT', "The word doing the work is 'non-minor'.", "A foul is a breach caused by non-minor contact between opposing players. So \"there was contact\" is never the whole argument \u2014 it is the start of one. Brushing shoulders on the way to a disc you caught cleanly anyway does not clear that bar.", ['15.1'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('nonminor_r', g_detail(5, [('15.1', [rt('15.1')])]), [0.3, 2.0]),
    ('threshold', g_main(6, 'DID IT ACTUALLY CHANGE ANYTHING?', "The call has a threshold, and it isn't 'I felt that'.", "On top of the definition sits a second filter, from the Spirit rules. Only call a breach significant enough to make a difference to the outcome of the action, or where a player's safety is at risk. Two questions worth asking before you speak.", ['1.3', '1.3.10'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('threshold_r', g_detail(7, [('1.3', [rt('1.3'), ('1.3.10', rt('1.3.10'))])]), [0.3, 2.0]),
    ('tip', g_tip(8, "If it didn't change what happened and nobody's hurt, let it go.", "None of this is permission to barge. Lesson 25 still stands: you must try to avoid contact. But consistently over-calling is itself a Spirit breach, and it empties a game faster than any bad throw."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 27), [0.3, 0.8, 1.0, 1.4]),
]
