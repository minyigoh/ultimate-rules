...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Pulls that go out of bounds: the brick", "Those crossed lines in the middle of the field finally make sense.", 47, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('choice', g_main(2, 'STRAIGHT OUT, YOU CHOOSE', "A pull that never lands in gives you a choice.", "If it sails out without first touching the field or an offensive player, you choose where the point starts: the brick mark nearest the end zone you are defending, or the central zone closest to where it went out.", ['7.12'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('choice_r', g_detail(3, [('7.12', [rt('7.12')])]), [0.3, 2.0]),
    ('mark', g_main(4, 'THE CROSSED LINES', "The brick mark is already on the field.", "It is a pair of crossed one-metre lines, midway between the sidelines, one end-zone length in from a goal line. There is one at each end. You take the one nearest the end zone you are defending.", ['2.5'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('mark_r', g_detail(5, [('2.5', [rt('2.5')])]), [0.3, 2.0]),
    ('landed', g_main(6, 'LANDED IN? NO CHOICE', "Land it in, roll it out, and the choice is gone.", "The brick is only on offer if the pull never touched down. If it lands in the field and then rolls out untouched, you start where it first crossed the perimeter line instead.", ['7.11'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('landed_r', g_detail(7, [('7.11', [rt('7.11')])]), [0.3, 2.0]),
    ('tip', g_tip(8, "Signal the brick before you touch the disc.", "The brick is only binding if it is signalled before the disc is picked up. One arm fully extended overhead, and call it. Do that on the walk in, while the disc is still lying there, and nobody has to argue about whether you claimed it in time."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 47), [0.3, 0.8, 1.0, 1.4]),
]
