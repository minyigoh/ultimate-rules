...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "Calling a time-out", "Who can call a time-out, and what happens to everybody's feet.", 48, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('who', g_main(2, 'AFTER THE PULL', "Only the player holding the disc can stop it.", "A team-mate on the sideline cannot call one. Nor can a defender. After the pull it is the thrower in possession, and nobody else. It lasts seventy-five seconds.", ['20.3'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('who_r', g_detail(3, [('20.3', [rt('20.3')])]), [0.3, 2.0]),
    ('signal', g_main(4, 'MAKE A T AND SAY IT', "The signal is the call.", "Form a T with both hands, or one hand and the disc if you are holding it, and call time-out so the other team hears you. Doing one without the other is how a call gets missed.", ['20.1'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('signal_r', g_detail(5, [('20.1', [rt('20.1')])]), [0.3, 2.0]),
    ('count', g_main(6, 'THE COUNT DOES NOT RESET', "A time-out does not buy a fresh stall count.", "The count picks up where it left off, capped at nine — not back at one. The exception is a swap: if a different player takes the mark, it restarts at one.", ['20.3.6'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('count_r', g_detail(7, [('20.3.6', [rt('20.3.6')])]), [0.3, 2.0]),
    ('tip', g_tip(8, "The offence picks its positions first. Use that.", "When the time-out ends the offence chooses positions and freezes, and only then does the defence choose. How many time-outs you get is not in the rulebook — your event's format sets it, so ask your captain."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 48), [0.3, 0.8, 1.0, 1.4]),
]
