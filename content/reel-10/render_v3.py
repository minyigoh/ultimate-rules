...
SCENES = [
    ('cover', g_cover(1, 'BEGINNER', "You're allowed to leave the field", "The disc can fly out and come back. So can you.", 10, size=84), [0.35, 0.55, 0.85, 1.05, 0.6]),
    ('back', g_main(2, 'THE DISC CAN COME BACK', "Outside the line isn't out yet.", "A throw that curves outside the sideline and back over the field never went out at all. The disc is still live the whole time, and you are explicitly allowed to run off the field to make a play on it.", ['11.6', '11.7'], 1), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('back_r', g_detail(3, [('11.7', [rt('11.7')])]), [0.3, 2.0]),
    ('out', g_main(4, 'WHAT ACTUALLY PUTS IT OUT', 'Ground, or an out-of-bounds receiver.', "Only two things end it: the disc touching out-of-bounds ground, or an offensive player who is out-of-bounds touching the disc. Once you have caught it, the disc simply inherits your status.", ['11.6'], 2), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('out_r', g_detail(5, [('11.6', [rt('11.6')])]), [0.3, 2.0]),
    ('def', g_main(6, 'DEFENDERS ARE ALWAYS IN', 'The rule only constrains the offence.', "A defender standing well outside the sideline can reach in and block the disc, and it is a clean block. Defensive players are never out-of-bounds — the out-of-bounds area is defined to exclude them.", ['11.2'], 3), [0.3, 0.45, 0.7, 1.5, 0.8]),
    ('def_r', g_detail(7, [('11.2', [rt('11.2')])]), [0.3, 2.0]),
    ('tip', g_tip(8, 'Chase the ones that look gone', "Discs drifting over the sideline are not lost. Curving one back in is a real throw, not an accident — keep running until it touches something."), [0.3, 0.45, 0.7, 1.7]),
    ('close', g_closing(9, 10), [0.3, 0.8, 1.0, 1.4]),
]
